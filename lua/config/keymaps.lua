...
map("n", "[d", function() vim.diagnostic.jump({ count = -1 }) end, desc(diag.diagnostic .. " Previous diagnostic"))
map("n", "]d", function() vim.diagnostic.jump({ count = 1 }) end, desc(diag.diagnostic .. " Next diagnostic"))
...
map("n", "]c", with_gitsigns(function(gs)
  if vim.wo.diff then
    vim.cmd.normal({ "]c", bang = true })
  else
    gs.nav_hunk("next")
  end
end), desc(git.change .. " Next hunk"))
...
end), desc(git.change .. " Previous hunk"))

map("n", "<leader>gs", with_gitsigns(function(gs)
  gs.stage_hunk()
end), desc(git.add .. " Stage hunk"))

map("n", "<leader>gr", with_gitsigns(function(gs)
  gs.reset_hunk()
end), desc(git.delete .. " Reset hunk"))

map("v", "<leader>gs", with_gitsigns(function(gs)
  gs.stage_hunk({ vim.fn.line("."), vim.fn.line("v") })
end), desc(git.add .. " Stage selection"))

map("v", "<leader>gr", with_gitsigns(function(gs)
  gs.reset_hunk({ vim.fn.line("."), vim.fn.line("v") })
end), desc(git.delete .. " Reset selection"))

map("n", "<leader>gS", with_gitsigns(function(gs)
  gs.stage_buffer()
end), desc(git.staged .. " Stage buffer"))

map("n", "<leader>gR", with_gitsigns(function(gs)
  gs.reset_buffer()
end), desc(git.delete .. " Reset buffer"))

map("n", "<leader>gp", with_gitsigns(function(gs)
  gs.preview_hunk()
end), desc(git.change .. " Preview hunk"))

map("n", "<leader>gb", with_gitsigns(function(gs)
  gs.blame_line({ full = true })
end), desc(git.blame .. " Blame line"))

map("n", "<leader>gd", with_gitsigns(function(gs)
  gs.diffthis()
end), desc(git.change .. " Diff this"))

map("n", "<leader>gD", with_gitsigns(function(gs)
  gs.diffthis("~")
end), desc(git.change .. " Diff against HEAD"))

map("n", "<leader>gt", with_gitsigns(function(gs)
  gs.toggle_current_line_blame()
end), desc(git.blame .. " Toggle blame"))

map("n", "<leader>gw", with_gitsigns(function(gs)
  gs.toggle_word_diff()
end), desc(git.change .. " Toggle word diff"))
